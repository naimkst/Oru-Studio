...
function localImageFilePath(src) {
  const generatedFilename = getGeneratedImageFilename(src);

  if (generatedFilename) {
    return generatedImageFilePath(generatedFilename);
  }
...
function localPublicImageExists(src) {
  const filePath = localImageFilePath(src);
...
function normalizeImageSrc(src, fallbackKey, offset = 0) {
  ...
  if (generatedFilename) {
    return generatedImageExists(generatedFilename)
      ? generatedImageApiSrc(generatedFilename)
      : fallbackThumbnailFor(fallbackKey || src, offset);
  }
