...
export async function generateBlogArticle({ topic, category, scheduledAt } = {}) {
  ...
  const thumbnail = await generateAndSaveBlogImage({
    prompt:
  ...
  for (const block of body) {
    if (block.type !== "image" || contentImageIndex >= contentImageLimit) {
      continue;
    }
    ...
    block.src = await generateAndSaveBlogImage({
      ...
      slug,
      kind: "content",
      index: contentImageIndex,
    });
  }

  if (contentImageIndex === 0) {
    const fallbackImage = {
      type: "image",
      text: "",
      items: [],
      ...
      src: await generateAndSaveBlogImage({
        prompt: `Original editorial image for ${article.title}, showing a practical Shopify merchant workflow and ecommerce development planning scene.`,
        title: article.title,
        slug,
        kind: "content",
        index: 1,
